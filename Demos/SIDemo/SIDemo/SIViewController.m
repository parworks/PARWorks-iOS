...
- (void)showAugmentingViewWithImage:(UIImage *)image
{
    ...
    int count = [SIUtil smallImagesWithWidth:WIDTH height:HEIGHT fromImage:_image withImageReadyCallback: ^(int i, UIImage* img) {
...
- (IBAction)translateLayersOffscreen
{
    ...
    for (int i=0; i<rows; i++) {
        for (int j=0; j<cols; j++) {
            int index = [SIUtil arrayIndexForCols:cols rowIndex:i columnIndex:j];
